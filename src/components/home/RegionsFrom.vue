<template>
  <div v-if="isDropdownOpen" @click="toggleDropdown" class="fixed top-0 left-0 h-screen w-screen z-10"></div>
  <div class="relative">
    <button
      class="text-[30px] w-[301px] block bg-second/35 relative text-white rounded-md"
      :class="!setPlacePinFrom ? 'px-[87px] py-7' : 'px-[15px] py-[11px]'"
      @click="toggleDropdown"
    >
      <div class="duration-300" :class="setPlacePinFrom ? 'scale-100' : 'h-0 w-0 scale-0'">
        <p class="text-[20px] text-start text-[#D1D1D1]">Qayerdan</p>
        <p class="line-clamp-1 mt-1 text-start">{{ setPlacePinFrom }}</p>
      </div>
      <div class="duration-300" :class="!setPlacePinFrom ? 'scale-100' : 'h-0 w-0 scale-0'">
        <p>Qayerga</p>
      </div>
    </button>
    <div :class="isDropdownOpen ? 'scale-100' : 'h-0 scale-0 w-0'" class="absolute left-0 duration-300 mt-2 w-[600px] bg-second/35 rounded shadow-lg z-10">
      <div class="flex">
        <!-- Region List -->
        ...
        <div v-if="selectedRegion !== null" class="w-1/2 max-h-56 overflow-y-auto scrollable-element">
          <p
            @click="setPlaceFromDistrict(regions[selectedRegion].region, district)"
...
const setPlacePinFrom = ref(null);
...
const setPlaceFromDistrict = (region, district) => {
  setPlacePinFrom.value = `${region}. ${district}`;
  console.log("object", setPlacePinFrom.value);
